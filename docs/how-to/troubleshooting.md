...
| GraphHopper exits with `No graph in /graph-cache` | The container never builds a graph by itself. Build it with `just build-graphhopper-graph-from FILE` ([rebuild](build-routing-graph.md#rebuild-on-the-vps)), or copy one [built elsewhere](build-routing-graph.md) into the graph cache. |
| The graph build stops with `Killed` and exit code 137 (`just`: `failed … with exit code 137`)  | The kernel killed it at `GRAPHHOPPER_MEM_LIMIT`, which must fit `GRAPHHOPPER_BUILD_HEAP` plus JVM overhead. Raise the limit, or for a large area build with `GRAPHHOPPER_BUILD_DATAACCESS=MMAP` ([large area on a small VPS](build-routing-graph.md#a-large-area-on-a-small-vps)) or [elsewhere](build-routing-graph.md#build-elsewhere-and-ship-it). |
...
| GraphHopper exits with `ROUTING_OSM_FILE_FILTERED is …, but … does not exist` or `… OSM_DATA_URL is no URL to download it from` | The filtered file was never imported here, or was deleted. Run `just osm-filter-many-raw-pbf-into-one` with the extracts again ([downloaded files](import-geodata.md)), copy the file into `ROUTING_OSM_IMPORT_DIR` (the log names it), or unset `ROUTING_OSM_FILE_FILTERED` and set `OSM_DATA_URL` to a download URL. |
| The build log shows `bike-bike-…` | `OSM_DATA_URL` names an already filtered file, so it is filtered again. Set `ROUTING_OSM_FILE_FILTERED` to that file instead and delete the `bike-bike-…` copy. |
| `just osm-filter-many-raw-pbf-into-one` stops with `Set ROUTING_OSM_FILE_FILTERED in .env…` | The import writes the file every build reads, so it needs its name: set `ROUTING_OSM_FILE_FILTERED`, e.g. `bike-europe-cycling.osm.pbf` ([step 2](import-geodata.md#2-set-env)). Nothing was changed. |
