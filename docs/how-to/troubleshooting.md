...
| GraphHopper stops with `No graph in /graph-cache`, the graph build stops with `Killed` / exit code 137 or `OutOfMemoryError`, a file is `not in ROUTING_OSM_IMPORT_DIR`, a `bike-bike-…` file appears, or a copied graph is refused | See [when something goes wrong](build-routing-graph.md#when-something-goes-wrong) in the routing-graph guide. |
| `just osm-filter-many-raw-pbf-into-one` stops with `Set ROUTING_OSM_FILE_FILTERED in .env…` | Set the name of the merged file it writes, e.g. `ROUTING_OSM_FILE_FILTERED=bike-europe-cycling.osm.pbf` ([path B](build-routing-graph.md#path-b-several-countries-merged-into-one)). Nothing was changed. |
| `just poi-extract` stops with `… is not built from OSM_DATA_URL`, or `Open failed for '/osm_data/…': No such file or directory` | The graph is merged from several countries, so there is no single raw extract to read. Copy the `pois-….geojsonseq` that `just osm-filter-many-raw-pbf-into-one` wrote into `ROUTING_OSM_IMPORT_DIR` and run `just poi-import` (`poi-import-prod` on the VPS), or copy the raw extracts there and name them: `just poi-extract germany-latest.osm.pbf …`. |
